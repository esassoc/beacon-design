...
| **`esa-badge` has no quiet/neutral variant and no slot.** All six variants are saturated fills; the only content channel is the `value` string, so it can't host a spinner or icon. | Unified Settings, three independent workarounds in one feature: `BcnOpCard` + `BcnSettingsSection` re-point `--badge-bg`/`--badge-text-color`; `BcnRunHistory`'s Status and Scope chips are cell renderers (its Running chip needs a spinner). `BcnCount` documented the same gap earlier. | Add a `neutral`/`quiet` variant (tint + mid-tone text, optional hairline) and a default slot alongside `value`. |
| **`esa-badge` success variant is dark-on-dark under theme-beacon.** The hub pairs the success fill with a lime foreground (`#37401c`); theme-beacon re-points `--color-success` to dark teal, so text lands dark-on-dark. | `BcnRunHistory` status badges, until the run history became a grid (a JS-injected cell can't host a compile-time `.astro` lego) and the local `--color-success-on-fill: var(--color-text-inverse)` fix left with them. Any success badge in the spoke still inherits the bug. | Theme-beacon should re-point the success *foreground* token alongside the fill, or the hub should derive on-fill color from the fill. |
| **No `esa-table` lego.** The hub ships no semantic-table chrome (header row, row hairlines, scroll box, fixed/auto layout). | Twice in this spoke, each with an identical justification comment: `BcnPrototypeIndex`, `BcnSettingsSection` (collection). `BcnRunHistory` was the third until it moved to the AG Grid kit — which is the answer wherever a list wants sorting and filters, not chrome. | A fully slotted `esa-table` owning chrome only — consumers keep their own body logic. |
| **`esa-alert-box` hangs ~6px of dead space under its icon.** `.esa-alert-box__icon` holds a bare inline `<svg>`, so the glyph sits on that div's text baseline and the inherited line box adds descender space below it: a one-line alert measures 53px where its text needs 47px, and the icon reads low against the line it labels. | Operations console — `BcnOpCard`'s all-tenants warning banner sat visibly taller than its single line. Patched with a scoped `:global(.esa-alert-box__icon svg) { display: block }`; every alert in every spoke carries the same 6px. | `display: block` on the icon `svg` (or `line-height: 0` / `display: flex` on `.esa-alert-box__icon`) in the lego. Nothing moves — the glyph is already flush to the top of its box. |
| **`esa-switch-toggle` host is `inline-block`, so the track floats high beside text.** The shadow button sits on a text baseline inside the host's inherited line box — the host grows taller than the track (descender space below), and any consumer centering the HOST centers the wrong box. | Unified Settings flag rows + setting rows: switches read visibly high against their 16px labels until both components forced `display:flex` on the host. | `:host { display: inline-flex; align-items: center; }` in the lego — kills the anonymous line box so host height = track height. |
| **No `esa-setting-row` lego, and no divided-card body.** `esa-form-field` is the opposite anatomy (label ABOVE control); `esa-card` has no divided-list body mode. | `BcnSettingRow` (label+description left, control right — six control kinds, all resolving to esa-* legos, zero brand values: strongest promotion candidate). The divided-card idiom is copied three times: `BcnSettingsSection` rows, `BcnFlagList`, `BcnLinkRowCard`. Also: `esa-text-field`/`esa-select`/`esa-textarea` expose no `::part(label)` handle (esa-switch-toggle does), which a row lego needs. | Promote a slotted `esa-setting-row` (label/description props + control slot) and a `divided` body mode on `esa-card`; add `::part(label)` to the form legos. |
