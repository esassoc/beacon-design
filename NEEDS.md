...
| **`esa-badge` has no quiet/neutral variant and no slot.** All six variants are saturated fills; the only content channel is the `value` string, so it can't host a spinner or icon. | Unified Settings, three independent workarounds in one feature: `BcnOpCard` + `BcnSettingsSection` re-point `--badge-bg`/`--badge-text-color`; `BcnRunHistory` hand-rolls a bordered span (its Running badge needs a spinner). `BcnCount` documented the same gap earlier. | Add a `neutral`/`quiet` variant (tint + mid-tone text, optional hairline) and a default slot alongside `value`. |
| **`esa-badge` success variant is dark-on-dark under theme-beacon.** The hub pairs the success fill with a lime foreground (`#37401c`); theme-beacon re-points `--color-success` to dark teal, so text lands dark-on-dark. | `BcnRunHistory` status badges — scoped a local `--color-success-on-fill: var(--color-text-inverse)` fix. Any success badge in the spoke inherits the bug. | Theme-beacon should re-point the success *foreground* token alongside the fill, or the hub should derive on-fill color from the fill. |
| **No `esa-table` lego.** The hub ships no semantic-table chrome (header row, row hairlines, scroll box, fixed/auto layout). | Third hand-rolled table in this spoke, each with an identical justification comment: `BcnPrototypeIndex`, `BcnSettingsSection` (collection), `BcnRunHistory`. | A fully slotted `esa-table` owning chrome only — consumers keep their own body logic. |
