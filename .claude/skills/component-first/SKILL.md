...
If — and only if — nothing exists in either tier, create a **real, reusable, documented** component with a `bcn-` class prefix. Not a one-off `.rd-`/`.tt-` page blob. See [bcn-authoring.md](bcn-authoring.md).
...
| `.rd-dropzone` + `.rd-files`/`.rd-file` rows + JS file list | **esa-file-upload** |
| `.rd-icon-btn` styled buttons | **esa-icon-button** |
| `.rd-eoc` card | **esa-card** |
| `.rd-tag` chips | **esa-pill** / **esa-chip-group** |
| `.rd-count` count badge | **esa-badge** |
| `.tt-empty` empty states | **esa-empty-state** |
| `.rd-field` / `.rd-field__label` wrappers | **esa-form-field** + **esa-field-error** |
