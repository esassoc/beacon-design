# Token contract — prototypes-setup-wizard

The 85 design tokens this page actually uses, resolved to their final values for the `beacon` theme. Component CSS still references them by name (`var(--color-background-brand)`), so the names carry the intent; the values below are what they currently resolve to.
...
| `--font-size-600` | `clamp(1.375rem, 1.2rem + .88vw, 1.875rem)` |
| `--icon-size-md` | `20px` |
| `--icon-size-sm` | `16px` |
| `--icon-size-xs` | `14px` |
