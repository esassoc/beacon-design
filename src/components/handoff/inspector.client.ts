...
/** Derive the bundle slug from the path. Bundles are keyed by the registry slug,
 * which is the route's LAST segment — so use the basename, not the whole path.
 * (beacon nests prototypes under /prototypes/<slug>; root routes still resolve to
