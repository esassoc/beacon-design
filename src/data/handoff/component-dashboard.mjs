  ...
  sections: [
    ...
    },
    {
      ...
      gotchas: [
        ...
        'Prod\'s Summary tab ends in an `<evidence-of-compliance-summary-grid [componentID]>` that this dashboard does NOT port — and the reason matters, because it looks like an omission. That grid renders "Highlighted Evidence of Compliance", not the component\'s evidence: its rows come from `dbo.ComponentSummaryEvidenceOfCompliance`, an explicit opt-in join. Its template short-circuits on `rowData.length > 0`, so with no rows it renders nothing at all — no heading, no empty state — which is why it is invisible in QA. And as of this writing there is no write path anywhere in develop: no UI references the join, and `EvidenceOfComplianceUpsertDto` has no field for it, so rows can only arrive by seed or direct insert. It is effectively dead UI. Confirm that still holds when slicing (an admin or import path may exist that this survey missed); if it does, dropping it is the finding, not a decision to agonise over. Note this leaves the dashboard with NO evidence surface — decide deliberately whether that is right, given the evidence drawer handles attaching and the Data Catalog lists evidence records.',
      ],
      acceptance: [
