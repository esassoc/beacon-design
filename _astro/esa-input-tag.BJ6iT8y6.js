import{i as n,b as t,A as l,a as d}from"./lit-element.C8p3bJxG.js";class c extends n{constructor(){super(),this.onDocClick=e=>{this._open&&(e.composedPath().includes(this)||this.closeDropdown())},this.onSearchInput=e=>{this._search=e.target.value,this._active=-1,this._open||this.openDropdown()},this.onInputFocus=()=>{this._open||this.openDropdown()},this.onKeydown=e=>{const i=this.filteredOptions,r=this.canAddTyped?1:0,o=i.length+r;switch(e.key){case"ArrowDown":if(e.preventDefault(),!this._open)return this.openDropdown();o>0&&(this._active=Math.min(this._active+1,o-1));break;case"ArrowUp":e.preventDefault(),o>0&&(this._active=Math.max(this._active-1,0));break;case"Enter":e.preventDefault(),this._open&&this._active>=0&&this._active<i.length?this.selectOption(i[this._active]):this.canAddTyped&&this.addToken(this._search);break;case"Escape":e.preventDefault(),this.closeDropdown(),this._search="";break;case"Backspace":!this._search&&this._values.length>0&&this.removeToken(this._values[this._values.length-1]);break}},this.toggleDropdown=()=>{this.disabled||(this._open?this.closeDropdown():this.openDropdown(),this._open&&this.focusInput())},this.label="",this.hint="",this.placeholder="Search or add...",this.options=[],this.size="md",this.disabled=!1,this.required=!1,this.strict=!1,this.tagsBelow=!1,this.name="",this._values=[],this._search="",this._open=!1,this._active=-1,this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={label:{type:String},hint:{type:String},placeholder:{type:String},options:{type:Array},size:{type:String,reflect:!0},disabled:{type:Boolean,reflect:!0},required:{type:Boolean},strict:{type:Boolean},tagsBelow:{type:Boolean,attribute:"tags-below"},name:{type:String},_values:{state:!0},_search:{state:!0},_open:{state:!0},_active:{state:!0}}}connectedCallback(){super.connectedCallback(),document.addEventListener("click",this.onDocClick),this.syncFormValue()}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("click",this.onDocClick)}set value(e){e==null?this._values=[]:Array.isArray(e)?this._values=[...e]:this._values=String(e).split(",").map(i=>i.trim()).filter(Boolean),this.syncFormValue()}get value(){return[...this._values]}labelFor(e){return this.options.find(r=>r.value===e)?.label??e}get filteredOptions(){const e=new Set(this._values),i=this._search.toLowerCase().trim();return this.options.filter(r=>!(e.has(r.value)||i&&!r.label.toLowerCase().includes(i)))}get canAddTyped(){if(this.strict)return!1;const e=this._search.trim();return!e||this._values.includes(e)?!1:!this.options.some(i=>i.label.toLowerCase()===e.toLowerCase()&&!this._values.includes(i.value))}syncFormValue(){this.internals.setFormValue(this._values.length?this._values.join(","):null)}emitValue(){this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0}))}openDropdown(){this.disabled||this._open||(this._open=!0,this._active=-1)}closeDropdown(){this._open&&(this._open=!1,this._active=-1)}focusInput(){requestAnimationFrame(()=>{this.renderRoot.querySelector(".input")?.focus()})}addToken(e){const i=e.trim();!i||this._values.includes(i)||(this._values=[...this._values,i],this._search="",this._active=-1,this.emitValue(),this.focusInput())}selectOption(e){this._values.includes(e.value)||(this._values=[...this._values,e.value],this._search="",this._active=-1,this.emitValue(),this.focusInput())}removeToken(e,i){i?.stopPropagation(),this._values=this._values.filter(r=>r!==e),this.emitValue(),this.focusInput()}renderChips(){return this._values.map(e=>t`<span class="chip">
        <span class="chip__label">${this.labelFor(e)}</span>
        ${this.disabled?null:t`<button
              type="button"
              class="chip__remove"
              aria-label=${"Remove "+this.labelFor(e)}
              @click=${i=>this.removeToken(e,i)}
            >
              ${this.xIcon()}
            </button>`}
      ...
      <div class="field">
        ...
        <div class="container ${this._open?"container--open":""} ${this.disabled?"container--disabled":""}">
          ...
          ${this.options.length>0?t`<button
                type="button"
                class="toggle"
        ...
        ${this.tagsBelow&&this._values.length?t`<div class="chips chips--below">${this.renderChips()}</div>`:null}
        ${this.hint?t`<span class="field__hint">${this.hint}</span>`:null}
      </div>
    `}renderDropdown(){const e=this.filteredOptions,i=this.canAddTyped,r=e.length;return e.length===0&&!i?t`<div class="dropdown" role="listbox">
        <div class="empty">${this._search?"No matches found":"Type a value and press Enter to add"}</div>
      </div>`:t`<div class="dropdown" role="listbox">
      ${e.map((o,s)=>t`<button
          type="button"
          class="option ${s===this._active?"option--active":""}"
          role="option"
      ...
      ${i?t`<button
            type="button"
            class="option option--add ${this._active===r?"option--active":""}"
            role="option"
      ...
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9" /></svg>`}xIcon(){return t`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>`}plusIcon(){return t`<svg class="option__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>`}static{this.styles=d`
    :host {
      display: block;
  ...
  `}}customElements.get("esa-input-tag")||customElements.define("esa-input-tag",c);
