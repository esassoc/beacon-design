import{a as e,n as t,s as n,t as r}from"./typography.Dt-sBkj4.js";var i={xs:`microcopy-xs-subtle`,sm:`microcopy-sm-subtle`,md:`microcopy-md-subtle`,lg:`microcopy-lg-subtle`},a={xs:`microcopy-xs`,sm:`microcopy-sm`,md:`microcopy-md`,lg:`microcopy-lg`},o=class extends t{static properties={tabs:{type:Array},activeIndex:{type:Number,attribute:`active-index`},size:{type:String,reflect:!0},variant:{type:String,reflect:!0},appearance:{type:String,reflect:!0}};constructor(){super(),this.tabs=[],this.activeIndex=0,this.size=`md`,this.variant=`underline`,this.appearance=`underline`}selectTab(e){this.tabs[e]?.disabled||(this.activeIndex=e,this.dispatchEvent(new CustomEvent(`tabchange`,{detail:{index:e},bubbles:!0,composed:!0})))}findNextEnabledTab(e,t){let n=e+t;for(;n>=0&&n<this.tabs.length;){if(!this.tabs[n].disabled)return n;n+=t}return null}onKeydown=(e,t)=>{let n=null;switch(e.key){case`ArrowRight`:n=this.findNextEnabledTab(t,1);break;case`ArrowLeft`:n=this.findNextEnabledTab(t,-1);break;case`Home`:n=this.findNextEnabledTab(-1,1);break;case`End`:n=this.findNextEnabledTab(this.tabs.length,-1);break;default:return}n!==null&&(e.preventDefault(),this.selectTab(n),(e.target.parentElement?.children[n])?.focus())};render(){return e`
      <div class="layout">
        <div class="tabs" part="tabs" role="tablist">
          ${this.tabs.map((t,n)=>{let r=this.activeIndex===n,o=r?a[this.size]:i[this.size];return e`<button
              class="tab typography-${o} ${r?`tab--active`:``} ${t.disabled?`tab--disabled`:``}"
              type="button"
              role="tab"
              aria-selected=${r}
              tabindex=${r?0:-1}
              ?disabled=${t.disabled}
              @click=${()=>this.selectTab(n)}
              @keydown=${e=>this.onKeydown(e,n)}
            >
              ${t.icon?e`<span class="icon" .innerHTML=${t.icon}></span>`:null}
              <span>${t.label}</span>
              ${t.badge==null?null:e`<span class="badge typography-microcopy-xs-strong">${t.badge}</span>`}
            </button>`})}
        </div>
  ...
  `]};customElements.get(`esa-tab-layout`)||customElements.define(`esa-tab-layout`,o);