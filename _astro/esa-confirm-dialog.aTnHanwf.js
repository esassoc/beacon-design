import{a as e,n as t,r as n,s as r,t as i}from"./typography.Dt-sBkj4.js";import{t as a}from"./boolish.oUPlfJhN.js";var o=class extends t{static properties={open:{type:Boolean,reflect:!0},heading:{type:String},message:{type:String},variant:{type:String,reflect:!0},confirmLabel:{type:String,attribute:`confirm-label`},cancelLabel:{type:String,attribute:`cancel-label`},showIcon:{type:Boolean,attribute:`show-icon`,converter:a},showCloseButton:{type:Boolean,attribute:`show-close-button`,converter:a}};constructor(){super(),this.open=!1,this.heading=``,this.message=``,this.variant=`default`,this.confirmLabel=`Confirm`,this.cancelLabel=`Cancel`,this.showIcon=!0,this.showCloseButton=!0}connectedCallback(){super.connectedCallback(),`closedBy`in HTMLDialogElement.prototype||this.addEventListener(`click`,this.onLightDismiss)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener(`click`,this.onLightDismiss)}get dialogEl(){return this.renderRoot.querySelector(`dialog`)}updated(e){if(!e.has(`open`))return;let t=this.dialogEl;t&&(this.open?(t.open||t.showModal(),requestAnimationFrame(()=>{this.renderRoot.querySelector(`.esa-confirm-dialog__btn--outline`)?.focus()})):t.close())}onNativeClose=()=>{this.open&&this.dismiss()};onLightDismiss=e=>{let t=this.dialogEl;if(!t||!this.open||e.composedPath()[0]!==t)return;let n=t.getBoundingClientRect();n.top<=e.clientY&&e.clientY<=n.top+n.height&&n.left<=e.clientX&&e.clientX<=n.left+n.width||this.dismiss()};show(){this.open=!0}resolve(e,t=!1){this.open=!1,t&&this.dispatchEvent(new CustomEvent(`dismiss`,{bubbles:!0,composed:!0})),this.dispatchEvent(new CustomEvent(e?`confirm`:`cancel`,{bubbles:!0,composed:!0})),this.dispatchEvent(new CustomEvent(`resolved`,{detail:{confirmed:e,dismissed:t},bubbles:!0,composed:!0}))}confirm=()=>this.resolve(!0);cancel=()=>this.resolve(!1);dismiss=()=>this.resolve(!1,!0);icon(){return this.showIcon?this.variant===`danger`?e`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>`:this.variant===`warning`?e`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>`:e`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>`:null}render(){return e`
      <dialog
        class="esa-confirm-dialog"
        role="alertdialog"
        closedby="any"
        aria-labelledby=${this.heading?`esa-confirm-dialog-title`:n}
        aria-label=${this.heading?n:`Confirm`}
        aria-describedby=${this.message?`esa-confirm-dialog-message`:n}
        @close=${this.onNativeClose}
      >
        ${``}
        ${this.showCloseButton?e`<button
        ...
        <div class="esa-confirm-dialog__content">
          ${this.showIcon?e`<div class="esa-confirm-dialog__icon esa-confirm-dialog__icon--${this.variant}">${this.icon()}</div>`:null}
  ...
  `]};customElements.get(`esa-confirm-dialog`)||customElements.define(`esa-confirm-dialog`,o);