...
>
  <circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line
    ...
    y2="16"
  />
</svg>`,s={xs:`label-2xs`,sm:`label-xs`,md:`label-md`,lg:`label-lg`},c={xs:`body-2xs`,sm:`body-xs`,md:`body-md`,lg:`body-lg`},l=class extends t{static formAssociated=!0;static properties={label:{type:String},size:{type:String,reflect:!0},placeholder:{type:String},helpText:{type:String,attribute:`help-text`},errorText:{type:String,attribute:`error-text`},required:{type:Boolean,reflect:!0},disabled:{type:Boolean,reflect:!0},name:{type:String,reflect:!0},rows:{type:Number},autoResize:{type:Boolean,attribute:`auto-resize`,reflect:!0},maxRows:{type:Number,attribute:`max-rows`},value:{type:String},minlength:{type:Number},maxlength:{type:Number},autocomplete:{type:String},liveError:{type:Boolean,attribute:`live-error`}};internals;warnedNameless=!1;constructor(){super(),this.label=``,this.size=`md`,this.placeholder=``,this.helpText=``,this.errorText=``,this.required=!1,this.disabled=!1,this.rows=3,this.autoResize=!1,this.maxRows=10,this.value=``,this.autocomplete=``,this.liveError=!1,this.internals=this.attachInternals()}connectedCallback(){super.connectedCallback(),this.internals.setFormValue(this.value)}updated(e){e.has(`value`)&&this.internals.setFormValue(this.value),this.syncValidity(),this.warnIfNameless()}syncValidity(){let e=this.textareaEl;if(!e)return;let t=e.validity;if(t.valid){this.internals.setValidity({});return}let n=t.valueMissing&&this.label?`Enter ${this.label}.`:e.validationMessage;this.internals.setValidity({valueMissing:t.valueMissing,tooLong:t.tooLong,tooShort:t.tooShort,badInput:t.badInput},n,e)}warnIfNameless(){this.warnedNameless||this.label||this.getAttribute(`aria-label`)||(this.warnedNameless=!0,console.warn("⚠️  esa-textarea has no accessible name. Set `label` (preferred — it renders visibly AND wires <label for>), or `aria-label`. `placeholder` is not a name: it vanishes as soon as the user types.",this))}get textareaEl(){return this.renderRoot?.querySelector(`textarea`)??null}onInput=e=>{this.value=e.target.value,this.internals.setFormValue(this.value),this.dispatchEvent(new CustomEvent(`change`,{detail:{value:this.value},bubbles:!0,composed:!0})),this.autoResize&&this.adjustHeight()};adjustHeight(){let e=this.textareaEl;if(!e)return;e.style.height=`auto`;let t=parseFloat(getComputedStyle(e).lineHeight)||20,n=this.maxRows*t,r=Math.min(e.scrollHeight,n);e.style.height=`${r}px`}focus(e){let t=this.renderRoot?.querySelector(`.input`);t?t.focus(e):super.focus(e)}render(){let t=!!this.errorText,r=[t?`error`:``,this.helpText?`help`:``].filter(Boolean).join(` `);return e`
      <div class="field ${t?`field--error`:``} ${this.autoResize?`field--auto`:``}">
        ${this.label?e`<label class="label typography-${s[this.size]}" for="input"
              >${this.label}${this.required?e`<span class="required" aria-hidden="true">*</span>`:null}</label
            >`:null}
        <textarea
          id="input"
