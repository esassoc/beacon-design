import{a as e,n as t,r as n,s as r,t as i}from"./typography.Dt-sBkj4.js";import{t as a}from"./a11y.CplpE3OP.js";import{t as o}from"./announcer.CEmnlEx3.js";var s=e`<svg
  class="error__icon"
  viewBox="0 0 24 24"
  fill="none"
  stroke="currentColor"
...
>
  <circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line
    ...
    y2="16"
  />
</svg>`,c={xs:`label-2xs`,sm:`label-xs`,md:`label-md`,lg:`label-lg`},l={xs:`microcopy-2xs-subtle`,sm:`microcopy-xs-subtle`,md:`microcopy-md-subtle`,lg:`microcopy-lg-subtle`},u={xs:`body-2xs`,sm:`body-xs`,md:`body-md`,lg:`body-lg`},d=!1,f=class extends t{static formAssociated=!0;static properties={mode:{type:String,reflect:!0},triggerStyle:{type:String,attribute:`trigger-style`},options:{type:Array},multiple:{type:Boolean},size:{type:String,reflect:!0},label:{type:String},placeholder:{type:String},disabled:{type:Boolean,reflect:!0},name:{type:String,reflect:!0},required:{type:Boolean},liveError:{type:Boolean,attribute:`live-error`},helpText:{type:String,attribute:`help-text`},cue:{type:String},errorText:{type:String,attribute:`error-text`},loading:{type:Boolean},debounceMs:{type:Number,attribute:`debounce-ms`},resultsCount:{type:Number,attribute:`results-count`},_search:{state:!0},_selected:{state:!0},_open:{state:!0},_active:{state:!0}};_suppressNextOpen=!1;warnedMode=!1;internals;searchTimer=null;lastEmittedSearch=``;onDocClick=e=>{this._open&&(e.composedPath().includes(this)||this.closeDropdown())};constructor(){super(),this.mode=`autocomplete`,this.triggerStyle=`field`,this.options=[],this.multiple=!1,this.size=`md`,this.label=``,this.placeholder=`Select...`,this.disabled=!1,this.required=!1,this.liveError=!1,this.helpText=``,this.cue=``,this.errorText=``,this.loading=!1,this.debounceMs=300,this.resultsCount=null,this._search=``,this._selected=[],this._open=!1,this._active=-1,this.internals=this.attachInternals()}connectedCallback(){super.connectedCallback(),document.addEventListener(`click`,this.onDocClick),this.syncFormValue(),this.warnDefaultModeFlip()}warnDefaultModeFlip(){d||this.hasAttribute(`mode`)||(d=!0,console.warn('⚠️  esa-combobox: `mode` now defaults to "autocomplete" (was "select" before 2026-08-15), so this instance renders a free-text input rather than a button trigger. If that is what you want, write `mode="autocomplete"` to silence this. If you wanted the button trigger over a fixed list, that is <esa-select>. (migrations.json: combobox-mode-select-to-select)'))}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener(`click`,this.onDocClick),this.searchTimer&&clearTimeout(this.searchTimer)}set value(e){this._selected=e==null?[]:Array.isArray(e)?e:[e],this.syncFormValue()}get value(){return this.multiple?this._selected:this._selected[0]??``}get filteredOptions(){let e=this._search.toLowerCase();return e?this.options.filter(t=>t.label.toLowerCase().includes(e)):this.options}get displayValue(){return this._selected.length===0?``:this.multiple?this.options.filter(e=>this._selected.includes(e.value)).map(e=>e.label).join(`, `):this.options.find(e=>e.value===this._selected[0])?.label??``}get selectedOptions(){return this.options.filter(e=>this._selected.includes(e.value))}get currentPlaceholder(){return this.multiple&&this._selected.length>0?``:this.placeholder}get inputValue(){return this.multiple?this._search:this._search||this.displayValue}isSelected(e){return this._selected.includes(e)}syncFormValue(){this.internals.setFormValue(this.multiple?this._selected.join(`,`):this._selected[0]??null)}updated(){this.syncValidity(),this.announceEmptyResults()}wasEmpty=!1;announceEmptyResults(){let e=this._open&&!this.loading&&!!this._search&&this.filteredOptions.length===0;e&&!this.wasEmpty&&o(`No results found`,{assertive:!0}),this.wasEmpty=e}get cueText(){return this.cue?this.cue:this.mode===`autocomplete`?`Results filter as you type. Use the up and down arrows to review them, Enter to choose.`:`Use the up and down arrows to review options, Enter to choose.`}syncValidity(){if(!this.required||this._selected.length>0){this.internals.setValidity({});return}let e=this.renderRoot?.querySelector(`.input, .trigger--field, .trigger`)??void 0;this.internals.setValidity({valueMissing:!0},this.label?`Select ${this.label}.`:`Select an option.`,e)}emitValue(){this.syncFormValue(),this.dispatchEvent(new CustomEvent(`change`,{detail:{value:this.value},bubbles:!0,composed:!0}))}emitSearch(e){this.searchTimer&&clearTimeout(this.searchTimer),this.searchTimer=setTimeout(()=>{e!==this.lastEmittedSearch&&(this.lastEmittedSearch=e,this.dispatchEvent(new CustomEvent(`search`,{detail:{term:e},bubbles:!0,composed:!0})))},this.debounceMs)}toggleDropdown(){this.disabled||(this._open?this.closeDropdown():this.openDropdown())}openDropdown(){this.disabled||this._open||(this._open=!0,this._active=-1,this.mode===`select`&&requestAnimationFrame(()=>{this.renderRoot.querySelector(`.search-input`)?.focus()}))}closeDropdown(){if(!this._open)return;let e=this.renderRoot,t=this.mode!==`autocomplete`&&!!e.activeElement;this._open=!1,this._search=``,t&&this.updateComplete.then(()=>{e.querySelector(`.trigger`)?.focus()})}selectOption(e){if(e.disabled)return;let t=e.value;if(this.multiple){let e=this._selected.indexOf(t);this._selected=e>=0?this._selected.filter(e=>e!==t):[...this._selected,t],this._search=``,this.emitValue();let n=this.mode===`autocomplete`?`.input`:`.search-input`;requestAnimationFrame(()=>this.renderRoot.querySelector(n)?.focus())}else if(this._selected=[t],this._search=``,this.emitValue(),this.closeDropdown(),this.mode===`autocomplete`){let e=this.renderRoot.querySelector(`.input`);e&&this.renderRoot.activeElement!==e&&(this._suppressNextOpen=!0,requestAnimationFrame(()=>e.focus()))}}removeValue(e,t){t?.stopPropagation(),this._selected=this._selected.filter(t=>t!==e),this.emitValue()}onSearchInput=e=>{let t=e.target.value;this._search=t,this._active=-1,this.emitSearch(t),this._open||this.openDropdown()};onInputFocus=()=>{if(this._suppressNextOpen){this._suppressNextOpen=!1;return}this._open||this.openDropdown()};onInputClick=()=>{this._open||this.openDropdown()};onKeydown=e=>{let t=this.filteredOptions;switch(e.key){case`ArrowDown`:if(e.preventDefault(),!this._open)return this.openDropdown();{let e=this._active+1;for(;e<t.length&&t[e].disabled;)e++;e<t.length&&(this._active=e)}break;case`ArrowUp`:if(e.preventDefault(),!this._open)return this.openDropdown();{let e=this._active-1;for(;e>=0&&t[e].disabled;)e--;e>=0&&(this._active=e)}break;case`Enter`:if(e.preventDefault(),this._open&&this._active>=0){let e=t[this._active];e&&!e.disabled&&this.selectOption(e)}else this._open||this.openDropdown();break;case`Escape`:e.preventDefault(),this.closeDropdown();break;case`Tab`:this.closeDropdown()}};highlight(t){let n=this._search.trim();if(!n)return e`${t}`;let r=t.toLowerCase().indexOf(n.toLowerCase());return r<0?e`${t}`:e`${t.slice(0,r)}<mark class="hl">${t.slice(r,r+n.length)}</mark>${t.slice(r+n.length)}`}focus(e){let t=this.renderRoot?.querySelector(`.input, .trigger`);t?t.focus(e):super.focus(e)}render(){let t=!!this.errorText;return e`
      <div class="field ${t?`field--error`:``}">
        ${this.label?e`<span class="field__label typography-${c[this.size]}" id="label">
    ...
    `}get describedBy(){return[this.errorText?`error`:``,this.helpText?`help`:``,`cue`].filter(Boolean).join(` `)}renderAutocomplete(){return e`
    ...
    `}renderChips(){return this.selectedOptions.length===0?n:e`<div class="chips">
      ${this.selectedOptions.map(t=>e`<span class="chip typography-body-sm">
          <span class="chip__label">${t.label}</span>
          <button
            type="button"
            class="chip__remove"
            aria-label=${`Remove `+t.label}
            @click=${e=>this.removeValue(t.value,e)}
          >
            ${this.xIcon()}
          </button>
        </span>`)}
    </div>`}renderDropdown(){let t=this.filteredOptions;return e`<div class="dropdown" role="listbox" id="listbox" @keydown=${this.onKeydown}>
      ${this.mode===`select`?e`<div class="search">
            ${this.searchIcon()}
            <input
              class="search-input typography-${u[this.size]}"
              placeholder="Search..."
              .value=${this._search}
              @input=${this.onSearchInput}
              @keydown=${this.onKeydown}
            />
            ${this.loading?e`<span class="spinner">${this.spinnerIcon()}</span>`:null}
          </div>`:null}

      ${this.resultsCount===null?null:e`<div class="results-count typography-body-sm">Displaying ${t.length} of ${this.resultsCount} results</div>`}

      <div class="viewport">
        ${t.map((t,r)=>{let i=this.isSelected(t.value);return e`<div
            class="option typography-${u[this.size]} ${r===this._active?`option--active`:``} ${i?`option--selected`:``} ${t.disabled?`option--disabled`:``}"
            role="option"
            id="opt-${r}"
            aria-selected=${i}
          ...
          >
            <span class="check ${i?`check--selected`:``}"
              >${this.checkIcon()}</span
            >
      ...
      ${t.length===0&&!this.loading?e`<div class="empty typography-${u[this.size]}">${this._search?`No results found`:`No options available`}</div>`:null}
      ${this.loading&&t.length===0?e`<div class="loading typography-${u[this.size]}"><span class="spinner">${this.spinnerIcon()}</span> Searching...</div>`:null}
    </div>`}chevronIcon(){return e`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      ...
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>`}searchIcon(){return e`<svg class="search__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>`}spinnerIcon(){return e`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>`}static styles=[i,a,r`
    :host {
      display: block;
  ...
  `]};customElements.get(`esa-combobox`)||customElements.define(`esa-combobox`,f);