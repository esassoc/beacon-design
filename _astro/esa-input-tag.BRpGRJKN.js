import{a as e,n as t,r as n,s as r,t as i}from"./typography.Dt-sBkj4.js";import{t as a}from"./a11y.CplpE3OP.js";import{t as o}from"./announcer.CEmnlEx3.js";var s={xs:`label-2xs`,sm:`label-xs`,md:`label-md`,lg:`label-lg`},c={xs:`microcopy-2xs-subtle`,sm:`microcopy-xs-subtle`,md:`microcopy-md-subtle`,lg:`microcopy-lg-subtle`},l={xs:`body-2xs`,sm:`body-xs`,md:`body-md`,lg:`body-lg`},u=class extends t{static formAssociated=!0;static properties={label:{type:String},helpText:{type:String,attribute:`help-text`},errorText:{type:String,attribute:`error-text`},hint:{type:String},placeholder:{type:String},options:{type:Array},size:{type:String,reflect:!0},disabled:{type:Boolean,reflect:!0},required:{type:Boolean},strict:{type:Boolean},tagsBelow:{type:Boolean,attribute:`tags-below`},name:{type:String,reflect:!0},_values:{state:!0},_search:{state:!0},_open:{state:!0},_active:{state:!0}};internals;warnedHint=!1;onDocClick=e=>{this._open&&(e.composedPath().includes(this)||this.closeDropdown())};constructor(){super(),this.label=``,this.helpText=``,this.errorText=``,this.hint=``,this.placeholder=`Search or add...`,this.options=[],this.size=`md`,this.disabled=!1,this.required=!1,this.strict=!1,this.tagsBelow=!1,this._values=[],this._search=``,this._open=!1,this._active=-1,this.internals=this.attachInternals()}get resolvedHelpText(){return this.hint&&!this.helpText?(this.warnedHint||(this.warnedHint=!0,console.warn(`⚠️  esa-input-tag: \`hint="${this.hint}"\` is deprecated — renamed to \`help-text="${this.hint}"\`. Run \`node ../ecology/scripts/migrate-tokens.mjs --write\` in your spoke (migrations.json: form-hint-to-help-text).`)),this.hint):this.helpText}connectedCallback(){super.connectedCallback(),document.addEventListener(`click`,this.onDocClick),this.syncFormValue()}updated(){this.syncValidity(),this.announceEmptyResults()}wasEmpty=!1;announceEmptyResults(){let e=this._open&&this.strict&&!!this._search.trim()&&this.filteredOptions.length===0;e&&!this.wasEmpty&&o(`No matching options`,{assertive:!0}),this.wasEmpty=e}syncValidity(){if(!this.required||this._values.length>0){this.internals.setValidity({});return}let e=this.renderRoot?.querySelector(`.input`)??void 0;this.internals.setValidity({valueMissing:!0},this.label?`Add at least one ${this.label}.`:`Add at least one value.`,e)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener(`click`,this.onDocClick)}set value(e){this._values=e==null?[]:Array.isArray(e)?[...e]:String(e).split(`,`).map(e=>e.trim()).filter(Boolean),this.syncFormValue()}get value(){return[...this._values]}labelFor(e){return this.options.find(t=>t.value===e)?.label??e}get filteredOptions(){let e=new Set(this._values),t=this._search.toLowerCase().trim();return this.options.filter(n=>!(e.has(n.value)||t&&!n.label.toLowerCase().includes(t)))}get canAddTyped(){if(this.strict)return!1;let e=this._search.trim();return!e||this._values.includes(e)?!1:!this.options.some(t=>t.label.toLowerCase()===e.toLowerCase()&&!this._values.includes(t.value))}syncFormValue(){this.internals.setFormValue(this._values.length?this._values.join(`,`):null)}emitValue(){this.syncFormValue(),this.dispatchEvent(new CustomEvent(`change`,{detail:{value:this.value},bubbles:!0,composed:!0}))}openDropdown(){this.disabled||this._open||(this._open=!0,this._active=-1)}closeDropdown(){this._open&&(this._open=!1,this._active=-1)}focusInput(){requestAnimationFrame(()=>{this.renderRoot.querySelector(`.input`)?.focus()})}addToken(e){let t=e.trim();!t||this._values.includes(t)||(this._values=[...this._values,t],this._search=``,this._active=-1,this.emitValue(),this.focusInput())}selectOption(e){this._values.includes(e.value)||(this._values=[...this._values,e.value],this._search=``,this._active=-1,this.emitValue(),this.focusInput())}removeToken(e,t){t?.stopPropagation(),this._values=this._values.filter(t=>t!==e),this.emitValue(),this.focusInput()}onSearchInput=e=>{this._search=e.target.value,this._active=-1,this._open||this.openDropdown()};onInputFocus=()=>{this._open||this.openDropdown()};onKeydown=e=>{let t=this.filteredOptions,n=+!!this.canAddTyped,r=t.length+n;switch(e.key){case`ArrowDown`:if(e.preventDefault(),!this._open)return this.openDropdown();r>0&&(this._active=Math.min(this._active+1,r-1));break;case`ArrowUp`:e.preventDefault(),r>0&&(this._active=Math.max(this._active-1,0));break;case`Enter`:e.preventDefault(),this._open&&this._active>=0&&this._active<t.length?this.selectOption(t[this._active]):this.canAddTyped&&this.addToken(this._search);break;case`Escape`:e.preventDefault(),this.closeDropdown(),this._search=``;break;case`Backspace`:!this._search&&this._values.length>0&&this.removeToken(this._values[this._values.length-1])}};toggleDropdown=()=>{this.disabled||(this._open?this.closeDropdown():this.openDropdown(),this._open&&this.focusInput())};renderChips(){return this._values.map(t=>e`<span class="chip typography-body-sm">
        <span class="chip__label">${this.labelFor(t)}</span>
        ${this.disabled?null:e`<button
              type="button"
              class="chip__remove"
              aria-label=${`Remove `+this.labelFor(t)}
              @click=${e=>this.removeToken(t,e)}
            >
              ${this.xIcon()}
            </button>`}
      </span>`)}focus(e){let t=this.renderRoot?.querySelector(`.input`);t?t.focus(e):super.focus(e)}render(){let t=!!this.errorText,r=this.resolvedHelpText,i=[t?`error`:r?`help`:``,`cue`].filter(Boolean).join(` `);return e`
      <div class="field ${t?`field--error`:``}">
        <!-- for="input" is load-bearing, not tidiness. Without it this label named
             ...
             cannot cross. -->
        ${this.label?e`<label for="input" class="field__label typography-${s[this.size]}">
        ...
        <div class="container ${this._open?`container--open`:``} ${this.disabled?`container--disabled`:``}">
          ...
          ${this.options.length>0?e`<button
                type="button"
                class="toggle"
        ...
        ${this.tagsBelow&&this._values.length?e`<div class="chips chips--below">${this.renderChips()}</div>`:null}
        ${t?e`<span class="field__error typography-body-sm" id="error">${this.errorText}</span>`:r?e`<span class="field__help typography-body-sm" id="help">${r}</span>`:null}
    ...
    `}renderDropdown(){let t=this.filteredOptions,n=this.canAddTyped,r=t.length;return t.length===0&&!n?e`<div class="dropdown" role="listbox">
        <div class="empty typography-${l[this.size]}">${this._search?`No matches found`:`Type a value and press Enter to add`}</div>
      </div>`:e`<div class="dropdown" role="listbox">
      ${t.map((t,n)=>e`<button
          type="button"
          class="option typography-${l[this.size]} ${n===this._active?`option--active`:``}"
          role="option"
          aria-selected=${n===this._active}
          @mousedown=${e=>e.preventDefault()}
      ...
      ${n?e`<button
            type="button"
            class="option option--add typography-${s[this.size]} ${this._active===r?`option--active`:``}"
            role="option"
      ...
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9" /></svg>`}xIcon(){return e`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>`}plusIcon(){return e`<svg class="option__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
      stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>`}static styles=[i,a,r`
    :host {
      display: block;
  ...
  `]};customElements.get(`esa-input-tag`)||customElements.define(`esa-input-tag`,u);