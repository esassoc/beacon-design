...
>
  <circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line
    ...
    y2="16"
  />
</svg>`,s={xs:`label-2xs`,sm:`label-xs`,md:`label-md`,lg:`label-lg`},c={xs:`microcopy-2xs-subtle`,sm:`microcopy-xs-subtle`,md:`microcopy-md-subtle`,lg:`microcopy-lg-subtle`},l=class extends t{static formAssociated=!0;static properties={label:{type:String},size:{type:String,reflect:!0},placeholder:{type:String},helpText:{type:String,attribute:`help-text`},errorText:{type:String,attribute:`error-text`},required:{type:Boolean,reflect:!0},disabled:{type:Boolean,reflect:!0},name:{type:String,reflect:!0},type:{type:String},value:{type:String},prefix:{type:String},suffix:{type:String},pattern:{type:String},minlength:{type:Number},maxlength:{type:Number},autocomplete:{type:String},inputmode:{type:String},liveError:{type:Boolean,attribute:`live-error`}};internals;warnedNameless=!1;constructor(){super(),this.label=``,this.size=`md`,this.placeholder=``,this.helpText=``,this.errorText=``,this.required=!1,this.disabled=!1,this.type=`text`,this.value=``,this.prefix=``,this.suffix=``,this.pattern=``,this.autocomplete=``,this.inputmode=``,this.liveError=!1,this.internals=this.attachInternals()}connectedCallback(){super.connectedCallback(),this.internals.setFormValue(this.value)}updated(e){e.has(`value`)&&this.internals.setFormValue(this.value),this.syncValidity(),this.warnIfNameless()}syncValidity(){let e=this.renderRoot?.querySelector(`.input`);if(!e)return;let t=e.validity;if(t.valid){this.internals.setValidity({});return}let n=t.valueMissing&&this.label?`Enter ${this.label}.`:e.validationMessage;this.internals.setValidity({valueMissing:t.valueMissing,typeMismatch:t.typeMismatch,patternMismatch:t.patternMismatch,tooLong:t.tooLong,tooShort:t.tooShort,rangeUnderflow:t.rangeUnderflow,rangeOverflow:t.rangeOverflow,stepMismatch:t.stepMismatch,badInput:t.badInput},n,e)}warnIfNameless(){this.warnedNameless||this.label||this.getAttribute(`aria-label`)||(this.warnedNameless=!0,console.warn("⚠️  esa-text-field has no accessible name. Set `label` (preferred — it renders visibly AND wires <label for>), or `aria-label` if the name is carried elsewhere. `placeholder` is not a name: it vanishes as soon as the user types.",this))}onInput=e=>{this.value=e.target.value,this.internals.setFormValue(this.value),this.dispatchEvent(new CustomEvent(`change`,{detail:{value:this.value},bubbles:!0,composed:!0}))};focus(e){let t=this.renderRoot?.querySelector(`.input`);t?t.focus(e):super.focus(e)}render(){let t=!!this.errorText,r=[t?`error`:``,this.helpText?`help`:``].filter(Boolean).join(` `);return e`
      <div class="field ${t?`field--error`:``}">
        ...
        <div class="control typography-${c[this.size]}">
          ${this.prefix?e`<span class="affix affix--prefix">${this.prefix}</span>`:null}
          <input
            id="input"
            class="input"
            .type=${this.type}
            ...
            @input=${this.onInput}
          />
          ${this.suffix?e`<span class="affix affix--suffix">${this.suffix}</span>`:null}
        </div>
