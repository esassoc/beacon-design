import{a as e,n as t,r as n,s as r,t as i}from"./typography.Dt-sBkj4.js";import{t as a}from"./boolish.oUPlfJhN.js";import{n as o,r as s}from"./overlay.DRAPVU52.js";var c=class extends t{static properties={position:{type:String,reflect:!0},trigger:{type:String},hasArrow:{type:Boolean,attribute:`has-arrow`,converter:a},offset:{type:Number},open:{type:Boolean,reflect:!0},appearance:{type:String,reflect:!0},label:{type:String}};showTimeout=null;constructor(){super(),this.position=`bottom`,this.trigger=`click`,this.hasArrow=!0,this.offset=8,this.open=!1,this.appearance=`default`,this.label=``}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener(`click`,this.onDocumentClick,!0),document.removeEventListener(`keydown`,this.onDocumentKeydown,!0),this.showTimeout&&clearTimeout(this.showTimeout)}show(){this.open||(this.open=!0,this.trigger===`click`&&document.addEventListener(`click`,this.onDocumentClick,!0),document.addEventListener(`keydown`,this.onDocumentKeydown,!0))}hide(){if(!this.open)return;this.open=!1,document.removeEventListener(`click`,this.onDocumentClick,!0),document.removeEventListener(`keydown`,this.onDocumentKeydown,!0);let e=o();e&&(this.contains(e)||this.renderRoot.contains(e))&&s(this.triggerEl)}get triggerEl(){let e=(this.renderRoot.querySelector(`slot:not([name])`)?.assignedElements({flatten:!0})??[]).find(e=>e instanceof HTMLElement);return e?e.matches(`a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])`)?e:e.querySelector(`a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])`)??e:null}syncTrigger(){let e=this.triggerEl;e&&(e.setAttribute(`aria-expanded`,String(this.open)),this.label&&e.setAttribute(`aria-haspopup`,`dialog`))}updated(){this.syncTrigger()}onDocumentKeydown=e=>{e.key===`Escape`&&this.open&&(e.preventDefault(),this.hide())};onTriggerClick=()=>{this.trigger===`click`&&(this.open?this.hide():this.show())};onMouseEnter=()=>{this.trigger===`hover`&&(this.showTimeout=setTimeout(()=>this.show(),200))};onMouseLeave=()=>{this.trigger===`hover`&&(this.cancelPending(),this.hide())};onFocusIn=()=>{this.trigger===`hover`&&(this.cancelPending(),this.show())};onFocusOut=e=>{if(this.trigger!==`hover`)return;let t=e.relatedTarget;t&&this.contains(t)||(this.cancelPending(),this.hide())};cancelPending(){this.showTimeout&&=(clearTimeout(this.showTimeout),null)}onDocumentClick=e=>{!this.contains(e.target)&&e.target!==this&&this.hide()};render(){return e`
      <div
  ...
  `]};customElements.get(`esa-popover`)||customElements.define(`esa-popover`,c);